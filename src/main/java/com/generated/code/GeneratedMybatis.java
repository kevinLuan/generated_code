...
public class GeneratedMybatis {
  ...
  static final String MYBATIS_INSERT =
      "<insert id='save' parameterType='%s' keyProperty='id_' useGeneratedKeys='true'>"//
          + "\n\t<![CDATA[\n\t%s\n\t]]>\n"//
          + "</insert>";
   ...
   * @return 文件名
   */
  public static String toXml(List<SimpleJavaType> javaTypes, String modelPack, String mapperPackage,
      StringBuilder builder) {
    SimpleJavaType javaType = javaTypes.get(0);
    // 名称使用GRPC生成的Model.builder
    String modelName = modelPack + "." + javaType.getClassName() + "$Builder";
    ...
    // <parameter property="startTime" typeHandler="com.look.mybatis.hander.DateTimeTypeHander"/>
    // </parameterMap>
    addParameterMap(javaTypes, modelPack, builder, javaType);
    addResultMap(javaTypes, modelPack, builder, javaType);
  ...
  private static void addParameterMap(List<SimpleJavaType> javaTypes, String modelPack,
      StringBuilder builder, SimpleJavaType javaType) {
  ...
  private static void addResultMap(List<SimpleJavaType> javaTypes, String modelPack,
      StringBuilder builder, SimpleJavaType javaType) {
